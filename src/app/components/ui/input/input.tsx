...
const VARIANTS = {
  ...
  primary: 'appearance-none px-4 py-2 border-px block w-full rounded-md border focus:outline-none',
  card: 'w-full app:text-black/ p-2 rounded-md bg-slate-50 focus:shadow-black/20'
}
...
export function Input({ className = '', variant = 'primary', ...props }: InputProps) {
  ...
  if (props.type === 'range') {
    ...
    return (
    <input className={cn(VARIANTS[variant], className)}
      {...props} />
  )
}