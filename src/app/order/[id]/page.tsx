...
<section className="h-screen flex items-center justify-center">
  ...
  orderInfo?.map((order, i)=>
    <div key={i} className="grid grid-cols-1 max-w-[1198px] gap-5 md:grid-cols-1 lg:grid-cols-2 xl:grid-cols-2">
    <div className="flex flex-col max-w-[583px] xl:w-[583px]">
      <h3 className="mb-3 text-text-color text-[20px] font-[700] leading-[25px]">Resumen del pedido</h3>
      <ul className="bg-color-details p-[32px]  rounded-[16px] flex flex-col gap-4">
      <li className=' pb-2 flex items-center justify-between w-full  border-b-[1px] border-b-color-separate'><p className=' text-text-color text-[18px] font-[700] leading-[22px]'>Importe:</p><p className=' text-text-color text-[18px] font-[700]'>{order.fiat_amount} {order.fiat}</p> </li>
      <li className=' pb-3 pt-3 flex items-center justify-between w-full  border-b-[1px]  border-b-color-separate'>
            <p className=' text-text-color text-[16px] font-[700]'>Moneda seleccionada:</p>
            <div className='flex gap-2 items-center justify-center'>
              <img className='w-[20px] h-[20px]' src={cryptoImg} alt='incono crypto'/>
              <p className=' text-text-color text-[16px] font-[700]'>{order.currency_id}</p>
            </div>
      </li>
          <li className=' pb-2 pt-3 flex items-center justify-between w-full border-b-[1px]  border-b-color-separate'>
              <div className='flex flex-col gap-3'>
                <p className=' text-text-color text-[16px] font-[700] leading-[22px]'>Comercio:</p>
                <p className=' text-text-color text-[16px] font-[700] leading-[22px]'>Fecha:</p>
              </div>
              <div className='flex flex-col items-end gap-3'>
                
                <div className='flex items-center justify-center'>
                  <Image src={CheckDetailPayment} alt={'check'}/>
                  <p className=' text-text-color text-[16px] font-[700] leading-[22px]'>Comercio de pruebas de Semega</p>
                </div>
                <p className=' text-text-color text-[16px] font-[700] leading-[22px]'>{setterDate(order.created_at,"DATE") }</p>
              </div>
          </li>
          <li className='pt-3 flex items-center justify-between w-full'>
              <p className=' text-text-color text-[16px] font-[700] leading-[22px]'>Concepto:</p>
              <div>
                <p className=' text-text-color text-[16px] font-[700] whitespace-pre-wrap text-right leading-[22px] line-clamp-3'>{order.notes}</p>
              </div>
        </li>
      </ul>
    </div>
    <div className="flex max-w-[583px] flex-col md:max-w-[583px] lg:w-[583px] xl:w-[583px]">
      <h3 className="mb-3 text-text-color text-[20px] font-[700] leading-[25px]">Realiza el pago</h3>
      <ul className="w-full p-[32px] bg-white flex flex-col gap-7 border rounded-[16px] shadow-lg">
      <li  className="w-full flex items-center justify-center">
        <div className='flex justify-center items-center' >
          <Image src={TimeClock} alt='time' />
          <p className=' text-text-color text-[12px] font-[600]'>{timeRemaining}</p>
        </div>
      </li>
      ...
      <li className="w-full flex items-center justify-center">
        <div className='flex justify-center items-center gap-4'>
          <p className=' text-text-color text-[14px] font-[600] leading-[22px] g'>
          Enviar     <span className='text-[20px] font-[700] '><span className='pl-1' id='amount'>{order.crypto_amount}</span> {order.currency_id}</span>
          </p> 
      ...
      <li className="w-full flex items-center justify-center">
        <div className='flex justify-center items-center gap-2'>
          <p id='address'  className=' text-text-color text-[14px] font-[400] leading-[22px] line-clamp-3'>{order.address}</p>
      ...
      <li className="w-full flex items-center justify-center">
        <div className='flex justify-center items-center gap-2'>
          ...
          <p  className=' text-text-color text-[12px] font-[600] leading-[22px]' >Etiqueta de destino: <span id='tag'>{order.tag_memo.length > 0 ? order.tag_memo : 'No posee una etiqueta'}</span></p>
